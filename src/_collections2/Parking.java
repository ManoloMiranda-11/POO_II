...
public class Parking {
	...
	public boolean entraCoche(Coche c) {
		if (listaCoches.size() < totalPlazas) {
			...
			if (!existe) {
				...
				// Actualizar el conteo de coches iguales (basado en equals, sin modificar el
				// método)
				...
				// Añadir al conjunto (nota: Set usa equals, por lo que sólo se guardará una
				// instancia por valores)
			...
			if (indice != -1) {
				...
				if (cochesIguales.containsKey(removido)) {
					...
					if (cantidad > 1) {
						...
						// Si el contador es 1, comprobar si aún queda alguna instancia igual en la
						// lista
	...
	public void reportColores() {
		...
		for (Map.Entry<Color, Integer> entry : cochesPorColor.entrySet()) {
			if (entry.getValue() > 0) {
				System.out.println("El coche de color " + entry.getKey().getNombreCompleto() + " se repite "
						+ entry.getValue() + (entry.getValue() == 1 ? " vez." : " veces."));
	...
	public void reportMarcas() {
		...
		for (Map.Entry<Marca, Integer> entry : cochesPorMarca.entrySet()) {
			if (entry.getValue() > 0) {
				System.out.println("El coche de marca " + entry.getKey().getNombreCompleto() + " se repite "
						+ entry.getValue() + (entry.getValue() == 1 ? " vez." : " veces."));
				total += entry.getValue();
			}
		}
	...
	public void reportCochesIguales() {
		...
		for (Map.Entry<Coche, Integer> entry : cochesIguales.entrySet()) {
			System.out.println(
					entry.getKey() + " se repite " + entry.getValue() + (entry.getValue() == 1 ? " vez." : " veces."));
			total += entry.getValue();
		}
		System.out.println("Total coches: " + total);
	}
